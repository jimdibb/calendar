...
dregex='([\d][\d][\d][\d]-[\d][\d]-[\d][\d])'
dregex=re.compile(dregex)

#convert kelvin to farenheit, not using it as we're asking for imperial format temp data
def k_to_f (temp):
...
def process_high_low(wdata, today):
	...
	for i in wdata['list']: # i is each indivdual 3 hour weather report
		thedate=dregex.search(i['dt_txt']) #grab the date from it
		d=thedate.group(0)
		if (debug):
			print (d)
			print (hash(d))
...
iconmap={ 'Clear':'icons/Sun-48.png', 'Rain':'icons/Rain-48.png', 
			'rain':'icons/Rain-48.png', 'Clouds':'icons/Cloud-50.png',
			'Snow':'icons/Snow-48.png'}
...
if (debug):
	...
	logfile= open("~pi/tmp/currentWeather.log", 'a')
	...
	with tag('body'):
		...
		with tag('table'):
			with tag('tr'):
				...
				with tag('td'):
					with tag('b'):
						if (i['weather'][0]['main'] in iconmap.keys()):
							with tag('img', src=iconmap[i['weather'][0]['main']]):
								pass
						else:
							text(i['weather'][0]['main']) #print the word if there's no picture
				with tag('td'):
					
					text("%i / %i" % (highlow[today.__str__()]['high'], highlow[today.__str__()]['low']))
			...
			for i in resp['list']:
				with tag('tr'):	
					...
					if newdate.search(i['dt_txt']):
						...
						with tag('td'):
							text("%i / %i" % (highlow[isodate]['high'], highlow[isodate]['low']))	
						d+=1
						if d>4:
							break
						isodate , day = days_future(today,d)
